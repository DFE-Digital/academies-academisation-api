...
public class LegacyProjectListGetQuery : ILegacyProjectListGetQuery
{
	...
	public async Task<LegacyApiResponse<LegacyProjectServiceModel>?> GetProjects(
		IEnumerable<string>? states, string? title, IEnumerable<string>? deliveryOfficers, int page, int count, int? urn, IEnumerable<string>? regions, IEnumerable<string>? applicationIds)
	{

		var (projects, totalCount) = await _projectListGetDataQuery.SearchProjects(
												states, title, deliveryOfficers, page, count, urn, regions, applicationIds);
