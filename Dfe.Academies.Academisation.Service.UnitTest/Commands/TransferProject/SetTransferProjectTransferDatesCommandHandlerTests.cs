...
namespace Dfe.Academies.Academisation.Service.UnitTest.Commands.TransferProject
{
	public class SetTransferProjectTransferDatesCommandHandlerTests
	{
		...
		[Fact]
		public async Task Handle_DerivesSfsoCommissioningRequestedDate_WhenTargetDateIsLessThan15DaysInFuture()
		{
			// Arrange
			var transferringAcademies = new List<TransferringAcademy>
			{
			...
			{
				Urn = 1,
				HtbDate = DateTime.Today.AddDays(40),
				TargetDateForTransfer = DateTime.Today.AddDays(14),
				IsCompleted = true
			};
